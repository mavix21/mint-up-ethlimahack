...
export const Faucet = () => {
  ...
  const [faucetAddress, setFaucetAddress] = useState<AddressType>(
    arbitrumNitro.accounts[0].address,
  );
        ...
        notification.error(
          <>
            <p className="font-bold mt-0 mb-1">
              No se puede conectar al proveedor local
            </p>
            <p className="m-0">
              - ¿Olvidaste ejecutar{" "}
              <code className="bg-muted font-bold italic">yarn chain</code>?
            </p>
            <p className="mt-1 break-normal">
              - También puedes cambiar{" "}
              <code className="bg-muted font-bold italic">targetNetwork</code>{" "}
              en{" "}
              <code className="bg-muted font-bold italic">
                scaffold.config.ts
              </code>
            </p>
          </>,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Faucet local</DialogTitle>
        </DialogHeader>
