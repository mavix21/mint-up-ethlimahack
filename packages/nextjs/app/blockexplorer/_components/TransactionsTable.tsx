  ...
  return (
    <div className="flex justify-center px-4 md:px-0">
      <Card className="w-full">
        <CardContent>
          {!hasTransactions ? (
            <div className="p-8 text-center text-muted-foreground">No transactions found on this page.</div>
          ) : (
            <Table data-testid="blockexplorer-table">
              <TableHeader>
                <TableRow>
                  <TableHead>Transaction Hash</TableHead>
                  <TableHead>Function Called</TableHead>
                  <TableHead>Block Number</TableHead>
                  <TableHead>Time Mined</TableHead>
                  <TableHead>From</TableHead>
                  <TableHead>To</TableHead>
                  <TableHead className="text-end">Value ({targetNetwork.nativeCurrency.symbol})</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {blocks.map(block =>
                  (block.transactions as TransactionWithFunction[]).map(tx => {
                    ...
                    return (
                      <TableRow key={tx.hash} data-testid="blockexplorer-row">
                        ...
                        <TableCell className="w-1/12 md:py-4" data-testid="blockexplorer-block-number">
                          {block.number?.toString()}
                        </TableCell>
                        <TableCell className="w-2/12 md:py-4">{timeMined}</TableCell>
                        <TableCell className="w-2/12 md:py-4">
