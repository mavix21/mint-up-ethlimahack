...
export async function generateTsAbi(
  ...
  const completeAbi = validateAndLoadCanonicalAbi(abiJson, contractName);

  await generateLocalAbiDefinition(completeAbi, contractName);
...
async function generateLocalAbiDefinition(
  abi: readonly unknown[],
  contractName: string,
) {
  const targetDirectory = path.resolve("scripts", "generated");
  const targetFile = path.join(targetDirectory, `${contractName}.ts`);
  fs.mkdirSync(targetDirectory, { recursive: true });
  const output = `/** This file is autogenerated by scaffold-stylus. */\nconst abi = ${JSON.stringify(
    abi,
  )} as const;\n\nexport default abi;\n`;
  fs.writeFileSync(
    targetFile,
...
export function validateAndLoadCanonicalAbi(
  abi: readonly unknown[],
  contractName: string,
) {
  type AbiEntry = {
  ...
  const key = (entry: AbiEntry) =>
    `${entry.type}:${entry.name ?? ""}:${entry.stateMutability ?? ""}:${(
      entry.inputs ?? []
    )
      ...
      .join(",")}`;
  const callableKeys = (entries: readonly AbiEntry[]) =>
    entries
      .filter((entry) => entry.type === "function" || entry.type === "error")
      .map(key)
      .sort();
  const exportedKeys = callableKeys(abi as readonly AbiEntry[]);
  const sourceKeys = callableKeys(sourceContract.abi);
  if (JSON.stringify(exportedKeys) !== JSON.stringify(sourceKeys)) {
    throw new Error(
      `Stylus export does not match the canonical Solidity interface at ${sourcePath}`,
    );
  }

  return sourceContract.abi;
}
